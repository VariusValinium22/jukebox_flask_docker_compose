"""
Populate jukebox database with data using the SQLAlchemy ORM.
"""
...
from jukebox.src.models import  db, Customers, Order_details, Orders, Albums, Songs 
from jukebox.src import create_app
...
def truncate_tables():
    """Delete all data/records from database tables"""
    # db.session.execute(likes_table.delete())
    Customers.query.delete()
    Order_details.query.delete()
    Orders.query.delete()
    Albums.query.delete()
    Songs.query.delete()
    db.session.commit()
